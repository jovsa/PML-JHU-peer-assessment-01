...
This report detials the building of machine learning models which predict the *classe* (quality of extercise) classification from data provided by accelerometer. To do this 5 classification models were initially selected and then the best of these 5 was fine tuned to improve the overall accuracy. The goal was to predict 5 exercise from 6 different participants. 
...
In order to improve overall accuracy while prevent overfitting, a k-fold cross validation (where k=10) was done while re-training the model. 

The code below detials the process.
...
Hence, we see that the *roll_belt* is vital for model rediction and the subsequent 19 variables are major contirbuters to model accuracy. 
